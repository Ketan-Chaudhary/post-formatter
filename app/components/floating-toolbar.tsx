...
export const FloatingToolbar = memo(function FloatingToolbar({
  ...
  return (
    <Card
      className="absolute z-50 p-3 shadow-xl border-2 bg-white dark:bg-gray-800 rounded-lg transition-all duration-200"
      style={{
        left: `${position.x}px`,
        top: `${position.y}px`,
      ...
      <div className="flex space-x-2">
        {toolbarButtons.map(({ style, icon: Icon, label, shortcut, example, className }) => (
          <Button
            ...
            } border-2 transition-colors duration-200 ${className || ""}`}
          >
            {isLoading ? (
              ...
              <div className="flex items-center">
                ...
                <Italic className="h-3 w-3 -ml-1" />
              </div>
            ) : (
              <Icon className={`h-4 w-4 ${style === "normal" ? "text-red-600 dark:text-red-400" : ""}`} />
            )}
